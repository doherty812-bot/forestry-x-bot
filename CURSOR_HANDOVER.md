...
| 夜20時コンテンツ方針 | **未決**（所有者確認待ち。無確認で変更しない） |
...
夜20時は、説明コメントやエントリーポイントには「産業・経営トレンド×林業への洞察」と残っています。一方、実際の `fetch_todays_buzz_article()` は、国内農林業系ニュースを選ぶ実装です。過去の依頼には「海外では〜」が多いため、20時投稿を国内農林業系ニュース引用へ変更する指示もあります。したがって、**現在の実装は国内農林業ニュース引用に寄っています**。再開前に、夜20時を「広い産業・経営トレンド」へ戻すのか、「国内農林業ニュース」へ固定するのかを、所有者に一度だけ確認してください。無確認で変更しないことが安全です。[3]
...
| 中 | 夜20時の要件表現と検索クエリが不整合 | 期待する話題の範囲が曖昧になる | 再開前に内容方針を確定し、関数名・docstring・プロンプト・検索クエリを同時に揃える |
...
夜20時のテーマは、次の二択を確認してください。第一の案は、現行に近い**国内農林業ニュースを引用し、現場目線のコメントを添える**方式です。第二の案は、幅広い**産業・経営トレンドを引用し、林業経営への示唆を述べる**方式です。いずれを選ぶかでRSSクエリと生成プロンプトが変わります。
